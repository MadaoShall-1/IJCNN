...
class FormulaRetriever:
    ...
    def retrieve(
        ...
        topic_candidates = self._topic_filter(parse_obj.domains, parse_obj.sub_domains)
        ...
        for step in formula_steps:
            ...
            scored = self._score_candidates(topic_candidates, canonical_vars, step_vars)
    ...
    def _score_candidates(
        ...
        """Score formula entries by overlap with the step's variable names.

        Score = (canonical overlap + 0.5 × direct symbol overlap) / denom,
        clipped to [0, 1].  A non-zero direct-symbol bonus rewards entries
        whose variable symbols exactly match the problem's variable names even
        when the canonical mapping failed.
        """
        scored = []
        for entry in candidates:
            ...
            # F1-style canonical overlap: harmonic mean of precision and recall.
            # This penalises entries whose canonical set only partially covers the
            # step's variables (low recall) without rewarding narrow entries that
            # happen to match one variable out of many they don't cover (low precision).
            ...
            # Small direct-symbol bonus — helps when canonicalization fails for short
            # conventional symbols (e.g. q1, q2 in Coulomb's law).
            direct_overlap = len(entry_symbols & direct_vars)
            direct_score = 0.2 * direct_overlap / max(len(direct_vars), 1)

            score = canonical_score + direct_score
