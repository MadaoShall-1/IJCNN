...
import type2.stage2 as stage2_module
...
# ---------------------------------------------------------------------------
# SymPy solve fallback
# ---------------------------------------------------------------------------

class TestSympySolveFallback(unittest.TestCase):

    def test_solves_ohms_law_current(self):
        step = StepObject(
            step_id="step_1",
        ...
        ok = _try_sympy_solve_step(
            step=step,
            formula_entry=_ohms_law_entry(),
            plan_output_var={"I": "I"},
            vso=vso,
        )
        ...
        self.assertAlmostEqual(step.output_var["I"], 0.5)
        self.assertIn("0.5", step.intermediate_answer)


# ---------------------------------------------------------------------------
# SolveTrace status handling
# ---------------------------------------------------------------------------

@unittest.skipUnless(hasattr(stage2_module, "SolveTrace"), "dspy not installed")
class TestSolveTraceStatus(unittest.TestCase):

    def test_generator_error_uses_sympy_fallback(self):
        parse_obj = _make_parse_obj(
            problem_text="A 10 ohm resistor has 5 V applied. Find the current.",
            known_quantities={
                "V": {"value": 5.0, "unit_symbol": "V", "unit_name": "volts"},
                "R": {"value": 10.0, "unit_symbol": "ohm", "unit_name": "ohms"},
            },
            step_plan=[
                {
                    "step_id": "step_1",
                ...
                {
                    "step_id": "step_2",
                    "goal": "Report the final value of I.",
                    "type": "conclusion",
                    "input_var": {"I": "I"},
                    "output_var": {"I": "I"},
                }
        ...
        formula_set = FormulaSet(
            formulas={"step_1": _ohms_law_entry(), "step_2": None},
            retrieval_confidence=0.9,
            path_index=0,
        )

        solver = stage2_module.SolveTrace()
        solver.generator = MagicMock(side_effect=RuntimeError("No LM is loaded."))

        trace = solver.forward(
            parse_obj=parse_obj,
        ...
        self.assertIn("0.5", trace.steps[0].intermediate_answer)
        self.assertIn("SymPy fallback", trace.steps[0].verifier_notes)
        self.assertEqual(trace.steps[1].status, "OK")
