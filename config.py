...
class SolverConfig:
    ...
    # Stage 0 parser recovery path. Keep API behavior aligned with the dataset
    # run command:
    #   python scripts/run_stage0_on_dataset.py --use-llm-fallback --output-dir outputs/stage0_with_llm_v2
    stage0_use_llm_fallback: bool = True
    # Reuse Stage 0 artifacts from a prior dataset run when possible. API
    # callers can override per request with `use_stage0_cache: false`.
    stage0_cache_enabled: bool = True
    stage0_cache_results_path: str = "outputs/stage0_with_llm_v2/stage0_results.jsonl"
    ...
    # DSPy LM. Leave model empty to avoid accidental remote calls; set these
    # via environment variables before starting uvicorn.
